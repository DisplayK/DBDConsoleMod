...
UGFxMoviePlayer::UGFxMoviePlayer()
{
	...
	this->CaptureKeys = TArray<FKey>();
	this->FocusIgnoreKeys = TArray<FKey>();
	this->ExternalTextures = TArray<FExternalTexture>();
	this->RenderTextureMode = EGFxRenderTextureMode::Opaque;
	this->Priority = 0;
	this->WidgetBindings = TArray<FGFxWidgetBinding>();
	this->SplitscreenLayoutYAdjust = 0;
	this->CachedMovieClipsArray = TArray<UGFxObject*>();
}
