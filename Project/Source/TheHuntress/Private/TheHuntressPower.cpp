#include "TheHuntressPower.h"

class ASlasherPlayer;

void ATheHuntressPower::StartHatchetShineCosmetic_Implementation()
{

}

void ATheHuntressPower::SetPercentThrowStrength(const float percentStrength)
{

}

void ATheHuntressPower::SetHatchetVisible(const bool visible)
{

}

bool ATheHuntressPower::IsHatchetThrowFullyCharged() const
{
	return false;
}

bool ATheHuntressPower::HasHatchet() const
{
	return false;
}

ASlasherPlayer* ATheHuntressPower::GetOwningKiller() const
{
	return NULL;
}

ATheHuntressPower::ATheHuntressPower()
{
	this->_reloadHatchetClass = NULL;
}
